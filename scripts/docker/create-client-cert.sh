...
export ORGANIZATIONAL_UNIT=$(docker compose -f ${WORK_DIR}/compose.yml exec tak-server bash -c "echo \$ORGANIZATIONAL_UNIT" | tr -d '\r')
export CAPASS=$(docker compose -f ${WORK_DIR}/compose.yml exec tak-server bash -c "echo \$CAPASS" | tr -d '\r')
export PASS=$(docker compose -f ${WORK_DIR}/compose.yml exec tak-server bash -c "echo \$PASS" | tr -d '\r')
...
docker compose -f ${WORK_DIR}/compose.yml exec tak-server bash -c "java -jar ${TAK_PATH}/utils/UserManager.jar usermod -p \"${USER_PASS}\" $USERNAME"

# Admin Priv
# docker compose -f ${WORK_DIR}/compose.yml exec tak-server bash -c "java -jar ${TAK_PATH}/utils/UserManager.jar certmod -A ${TAK_PATH}/certs/files/${USERNAME}.pem"
